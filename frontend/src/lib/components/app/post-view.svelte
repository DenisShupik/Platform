<script lang="ts">
	...
	import { IconClockFilled, IconPencil } from '@tabler/icons-svelte'
	...
	let {
		...
		author,
		children
	}: { post: PostDto; author: UserDto; children: Snippet<[]> | undefined } = $props()
</script>
...
>
	...
	<div>
		<header class="bg-muted/40 flex h-9 w-full items-center gap-1 px-2 py-0 text-base">
			...
			{@render children?.()}
			<span class="text-muted-foreground">#{post.postId}</span>
		</header>
