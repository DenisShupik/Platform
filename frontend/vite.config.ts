...
function camelToKebab(value: string) {
	...
	return value.replace(/([a-zA-Z])(?=[0-9])|([a-z0-9])(?=[A-Z])/g, '$&-').toLowerCase() // Convert the entire string to lowercase.
}
...
function tablerIconsTreeShakePlugin() {
	return {
...
export default defineConfig({
	...
	resolve: {
		alias: {
			...
			'@tabler/icons-svelte/icons': path.resolve(
				__dirname,
