...
public static class HttpContextExtensions
{
    ...
    public static UserIdRole GetRequiredUserIdRole(this HttpContext context)
    {
        ...
        Role? role = null;
        {
            foreach (var roleClaim in user.Claims.Where(e => e.Type == ClaimTypes.Role).Select(e => e.Value))
            {
                if (!Enum.TryParse<Role>(roleClaim, out var r)) continue;
                if (role == null || r > role) role = r;
            }
        }
