...
{
    [Sql.Expression("websearch_to_tsquery('russian', {0})", ServerSideOnly = true)]
    public static NpgsqlTsQuery WebSearchToTsQuery(string term) => throw new InvalidOperationException();

    [Sql.Expression("to_tsquery('russian', {0} || ':*')", ServerSideOnly = true)]
    ...
    [Sql.Expression("ts_headline('russian', {0}, {1}, 'StartSel=⟦, StopSel=⟧, MaxWords=35, MinWords=15, MaxFragments=2')", ServerSideOnly = true)]
