...
public sealed class
    GetForumsBulkQuery<T> : IQuery<Dictionary<ForumId, Result<T, ForumNotFoundError>>>
    where T : notnull
{
    /// <summary>
...
public sealed class GetForumsBulkQueryHandler<T> : IQueryHandler<GetForumsBulkQuery<T>,
    Dictionary<ForumId, Result<T, ForumNotFoundError>>>
    where T : notnull
{
    ...
    public Task<Dictionary<ForumId, Result<T, ForumNotFoundError>>> HandleAsync(GetForumsBulkQuery<T> query,
        CancellationToken cancellationToken) => _repository.GetBulkAsync(query, cancellationToken);
}