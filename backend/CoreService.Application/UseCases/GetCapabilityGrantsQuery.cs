...
public sealed class GetCapabilityGrantsQuery : IQuery<Result<
...
public sealed class GetCapabilityGrantsQueryHandler(
    IAuthorizationScopeResolver scopes,
    ICapabilityGrantRepository grants,
    IForumPolicyEvaluator policies) : IQueryHandler<GetCapabilityGrantsQuery, Result<
    IReadOnlyList<CapabilityGrantDto>,
    PermissionDeniedError,
    AuthorizationScopeNotFoundError,
    InvalidAuthorizationScopeError>>
{
    public async Task<Result<
        ...
        GetCapabilityGrantsQuery query,
        CancellationToken cancellationToken)
    {
        ...
        var result = query.IncludeInactive
            ? await grants.GetDirectGrantHistoryAsync(scope, cancellationToken)
            : await grants.GetActiveDirectGrantsAsync(scope, query.EvaluatedAt, cancellationToken);
