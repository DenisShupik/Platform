...
public sealed partial class
    GetPostQuery<T> : IQuery<Result<T, PolicyViolationError, PolicyRestrictedError, PostNotFoundError>> where T : notnull
{
    public required UserId? QueriedBy { get; init; }
}

public sealed class GetPostQueryHandler<T> : IQueryHandler<GetPostQuery<T>,
    Result<T, PolicyViolationError, PolicyRestrictedError, PostNotFoundError>> where T : notnull
{
    ...
    public async Task<Result<T, PolicyViolationError, PolicyRestrictedError, PostNotFoundError>> HandleAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!accessCheckResult.TryOrExtend<T, PostNotFoundError>(out var accessErrors))
