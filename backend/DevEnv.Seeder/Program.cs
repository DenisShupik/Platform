...
builder.Services.AddHttpClient("moderator", httpClient => { httpClient.BaseAddress = apiGatewayUri; })
    .ConfigurePrimaryHttpMessageHandler(sp =>
    {
        var userTokenService = sp.GetRequiredService<UserTokenService>();
        var handler = new UserTokenService.Handler(userTokenService, () => "moderator");
        handler.InnerHandler = new HttpClientHandler();
        return handler;
    });
