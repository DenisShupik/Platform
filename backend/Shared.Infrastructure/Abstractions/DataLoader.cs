...
public abstract class DataLoader<TKey, TKeyPrimitive, TValue> : IDisposable
    where TKey : struct, IId, IHasTryFrom<TKey, TKeyPrimitive>, IVogen<TKey, TKeyPrimitive>
    where TKeyPrimitive : ISpanParsable<TKeyPrimitive>
{
    ...
    protected abstract Task<Dictionary<TKey, TValue>> FetchAsync(IdSet<TKey, TKeyPrimitive> keys,
        CancellationToken cancellationToken);

    public Task<TValue> LoadAsync(TKey key)
    {
        if (_disposed) throw new ObjectDisposedException(nameof(DataLoader<,,>));
        ...
        {
            tcs.TrySetException(new ObjectDisposedException(nameof(DataLoader<,,>)));
        }
    ...
    private async Task WorkerLoopAsync()
    {
        ...
        try
        {
            while (await reader.WaitToReadAsync(_cts.Token).ConfigureAwait(false))
            {
                ...
                var keys = IdSet<TKey, TKeyPrimitive>.TryCreate(map.Keys);
